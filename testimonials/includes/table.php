<!-- <div class="st-table">
    <div class="st-table-head">
        <div class="st-table-row">
            <div class="st-table-cell">

            </div>
            
        </div>
    </div>
</div> -->

<table class="mb-0 table">
    <thead>
        <tr>
            <th width="35">
                <label class="st-checkbox st-checkbox-primary d-inline-flex cursor-pointer">
                    <input
                        type="checkbox"
                        class="d-none" 
                    />
                    <span class="box d-flex align-items-center justify-content-center border" style="height: 17px; width: 17px">
                        <i class="feather-check st-fs-11 icon position-relative"></i>
                    </span>
                </label>
            </th>
            <th>
                <p class="text-capitalize">
                    Image
                </p>
            </th>

            <th>
                <p class="text-capitalize">
                    Name
                </p>
            </th>

            <th>
                <p class="text-capitalize">
                    Updated At
                </p>
            </th>

            <th>
                <p class="text-capitalize">
                    Status
                </p>
            </th>

            <th>
                <p class="text-capitalize">
                    Review
                </p>
            </th>
            
            <th width="90"></th>
        </tr>
    </thead>

    <tbody>
        <tr>
            <td class="column__checkbox">
                <label class="st-checkbox st-checkbox-primary d-inline-flex cursor-pointer">
                    <input
                        type="checkbox"
                        class="d-none" 
                    />
                    <span class="box d-flex align-items-center justify-content-center border" style="height: 17px; width: 17px">
                        <i class="feather-check st-fs-11 icon position-relative"></i>
                    </span>
                </label>
            </td>

            <td class="column__img">
                <div class="img">
                    <img src="https://www.commercey.com/stores/ruhstaging/testimonials/slide-1641475925.jpg" alt="testimonial-img" class="img-fluid" height="50" width="50">
                </div>
            </td>

            <td class="column__name"><p class="">Name</p></td>

            <td class="column__updatedAt"><p class="">Updated At</p></td>

            <td class="column__status"><p class="">Status</p></td>

            <td class="column__review"><p class="">Review</p></td>
            
            <td class="column__actions">
                <div class="d-flex justify-content-end">
                    <a 
                        href="https://www.commercey.com/stores/ruhstaging/products/8/6a/7183/small/7183-acacia-cutting-board499526873944.jpg"
                        class="st-round-btn st-btn-transparent st-btn-xs d-flex align-items-center justify-content-center me-1"
                        data-toggle="tooltip"
                        data-placement="right"
                        title="Edit">
                        <i class="feather-edit-2 icon"></i>                    
                    </a>

                    <button 
                        class="st-round-btn st-btn-transparent st-btn-xs d-flex align-items-center justify-content-center"
                        data-toggle="tooltip"
                        data-placement="right"
                        title="Delete">
                        <i class="feather-trash icon"></i>                    
                    </button>
                </div>
            </td>
        </tr>
    </tbody>
</table>